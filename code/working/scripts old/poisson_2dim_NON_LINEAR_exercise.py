...
def g(x):
    X, Y = x[0], x[1]
    return - np.exp(3*np.multiply(X, Y)) * ( (2 * np.multiply(X, Y)) + (3 * (np.multiply(np.square(X), np.square(Y)) + np.square(np.square(X))) + np.square(np.square(Y)) ) )
...
# initial u 
u = - np.multiply(np.square(X), Y)
# soution u
solution_u = np.exp(np.multiply(X, Y))
solution_u_interp = scipy.interpolate.interp2d(x, y, solution_u, kind="linear")

"""def cstm_phi_2d(vert, x):
    if vert[0] == 0 or vert[0] == 1 or vert[1] == 0 or vert[1] == 1:
        return np.exp(np.multiply(x[0], x[1])) * bf.hat
    else:
        return bf.phi_2d(vert, x, h)
"""    
...
def f_integrand(u, vert, x):
    grad_phi_x = np.gradient(cstm_phi_2d(vert, x), axis=0)
    grad_phi_y = np.gradient(cstm_phi_2d(vert, x), axis=1)
...
def J_integrand(u, vert0, vert1, x):
    grad_phi_i_x = np.gradient(cstm_phi_2d(vert0, x), axis=0)
    grad_phi_i_y = np.gradient(cstm_phi_2d(vert0, x), axis=1)
    grad_phi_j_x = np.gradient(cstm_phi_2d(vert1, x), axis=0)
    grad_phi_j_y = np.gradient(cstm_phi_2d(vert1, x), axis=1)
    grad_u_x = np.gradient(u, axis=0)
    grad_u_y = np.gradient(u, axis=1)
    
    summand_1_part_1 = 2*((grad_u_x * grad_phi_i_x) + (grad_u_y * grad_phi_i_y))
    summand_1_part_2 = np.multiply(grad_u_x, grad_phi_i_x) + np.multiply(grad_u_y, grad_phi_j_y)
    
    summand_1 = summand_1_part_1 * summand_1_part_2
    
    summand_2_part_1 = np.square(grad_u_x) + np.square(grad_u_y) 
    summand_2_part_2 = np.multiply(grad_phi_j_x, grad_phi_i_x) + np.multiply(grad_phi_i_y, grad_phi_j_y)
...
for t in range(1000):
    ...
    torch.save(u_iterations, "converging_func_test.pt")